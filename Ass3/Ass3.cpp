  ...
  struct FunctionAnalysisPass : public FunctionPass {
    ...
    bool runOnFunction(Function &F) override {
      ...
      for (BasicBlock &BB : F) {
        ...
        // Insert store instruction in each basic block to assign the counter value to the global variable
        if (&BB != &EntryBB) { // Skip the entry block to avoid overwriting the initial zero
          Builder.SetInsertPoint(&BB, BB.getFirstInsertionPt());
          Builder.CreateStore(ConstantInt::get(Type::getInt32Ty(Context), counter), gVar);
          counter++;
        }
