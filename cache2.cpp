...
public:
    ...
    SetAssociativeCache(int sets, int size, int assoc) : numSets(sets), setSize(size / sets) {
        //sets based on the number of sets specified
        for (int i = 0; i < numSets; ++i) {
            this->sets.emplace(i, new Set(assoc)); // Add new set to the cache
        }
    }
...
int main() {
    int cacheSize = 4;          // Total cache size
    int numSets = 2;            // Number of sets in the cache
    int setSize = 3;            // Associativity 
    SetAssociativeCache cache(numSets, cacheSize, setSize);
